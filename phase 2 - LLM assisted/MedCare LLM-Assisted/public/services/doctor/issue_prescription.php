<?php
/**
 * public/services/doctor/issue_prescription.php
 *
 * Service registrato come 'doctor_issue_prescription'.
 * GET  -> form per selezionare un paziente (tra quelli con relazione
 *         clinica presso questo medico) ed emettere una prescrizione.
 * POST -> inserisce la riga in prescriptions, opzionalmente collegata a
 *         un referto specifico (log_id).
 */
...
requireService('doctor_issue_prescription');
 ...
 * Stessa logica di dominio usata in view_patient_history.php e
 * update_medical_log.php.
 */
...
if ($doctorId !== false && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $logId = filter_input(INPUT_POST, 'log_id', FILTER_VALIDATE_INT) ?: null;
    $medication = trim((string)($_POST['medication'] ?? ''));
    $dosage = trim((string)($_POST['dosage'] ?? ''));
    $instructions = trim((string)($_POST['instructions'] ?? ''));

    if (!$patientId || $medication === '' || $dosage === '') {
        $feedback = 'Seleziona un paziente e compila farmaco e dosaggio.';
        $feedbackType = 'error';
    } elseif (mb_strlen($medication) > 150 || mb_strlen($dosage) > 100) {
        $feedback = 'Farmaco o dosaggio superano la lunghezza massima consentita.';
        $feedbackType = 'error';
    } elseif (mb_strlen($instructions) > 255) {
        $feedback = 'Le istruzioni sono troppo lunghe (massimo 255 caratteri).';
        $feedbackType = 'error';
    } elseif (!doctorHasRelationWithPatient($pdo, (int)$doctorId, $patientId)) {
        logAudit($userId, 'doctor_issue_prescription', 'ACCESS_DENIED');
        denyAccess('Non hai una relazione clinica con questo paziente.');
    } else {
        try {
            $stmt = $pdo->prepare(
                'INSERT INTO prescriptions (patient_id, doctor_id, log_id, issued_at, medication, dosage, instructions)
                 VALUES (:patient_id, :doctor_id, :log_id, NOW(), :medication, :dosage, :instructions)'
            );
            $stmt->execute([
            ...
            $feedback = 'Prescrizione emessa con successo.';
            $feedbackType = 'success';
        } catch (Throwable $e) {
            error_log('[issue_prescription] ' . $e->getMessage());
...
$tpl = new Template(dirname(__DIR__, 3) . '/skins/frontend/doctor/issue_prescription');
...
renderHeader('Nuova prescrizione');
echo $tpl->get();
renderFooter();
